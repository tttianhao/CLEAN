...
def NPairLoss(model_emb, temp):
    '''
    return the n-pair loss for back prop
    features:  
        model output embedding, dimension [bsz, n_all, out_dim], 
        where bsz is batchsize, n_all is anchor, pos, neg (n_all = 1+1+n_neg)
    ...
    # exp_logits is of size [bsz, 1+n_neg]
    exp_logits = torch.exp(logits[:, 1:])
    exp_logits_sum = exp_logits.sum(1) # size [bsz]
    # positive examples dot product z_i dot z_p, and z_p is the second element
    pos_logits = logits[:, 1]
    log_prob = pos_logits - exp_logits_sum
    # loss is averaged over negative log probability 
    loss = - log_prob.mean()
    return loss
...
def SupConHardLoss(model_emb, temp, n_pos):
    ...
    exp_logits = torch.exp(logits[:, 1:])
    exp_logits_sum = n_pos * exp_logits.sum(1) # size [bsz], scale by n_pos
...
def SupConHardLoss(model_emb, temp, n_pos):
    ...
    # anchor_dot_features now [bsz, n_all], contains 
    anchor_dot_features = anchor_dot_features.squeeze(1)
    # deduct by max logits, which will be 1/temp 
    logits = anchor_dot_features - 1/temp
    # the exp(z_i dot z_a) excludes the dot product between itself
    # exp_logits is of size [bsz, n_pos+n_neg]
    exp_logits = torch.exp(logits[:, 1:])
